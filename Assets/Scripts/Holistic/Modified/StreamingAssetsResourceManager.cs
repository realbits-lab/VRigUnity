...
namespace HardCoded.VRigUnity
{
  public class StreamingAssetsResourceManager : ResourceManager
  {
    ...
    {
      Logger.Info(_TAG, $"{path} will be used.");
    ...
    public StreamingAssetsResourceManager() : this("mediapipe") { }
    ...
    private IEnumerator CreateCacheFile(string assetName)
    {
      ...
      using (var webRequest = UnityWebRequest.Get(assetPath))
      {
        ...
        if (webRequest.result == UnityWebRequest.Result.Success)
        {
          ...
          Logger.Verbose(_TAG, $"Writing {assetName} data to {cacheFilePath}...");
          var bytes = webRequest.downloadHandler.data;
          File.WriteAllBytes(cacheFilePath, bytes);
          Logger.Verbose(_TAG, $"{assetName} is saved to {cacheFilePath} (length={bytes.Length})");
        }
        else
        {
